...
Этот репозиторий — только каталог (`.claude-plugin/marketplace.json`): он перечисляет три плагина и указывает, из каких репозиториев их брать; при установке файлы скилла подтягиваются из его собственного репозитория, копий здесь нет. Каталог закрепляет проверенный коммит каждого скилла, а робот (`.github/workflows/sync.yml`) каждые шесть часов сверяет закрепления с репозиториями скиллов и **обновляет их автоматически** — вслед за релизами.
