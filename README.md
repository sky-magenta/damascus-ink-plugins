...
Этот репозиторий — маркетплейс со встроенными копиями трёх скиллов (`plugins/…`): при установке всё берётся отсюда, внешние источники не подтягиваются. Первоисточник каждого скилла — его собственный репозиторий; копии обновляются вместе с релизами скиллов (зафиксированный коммит записан в `plugins/<имя>/.vendored-sha`).
