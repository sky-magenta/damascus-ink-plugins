...
Этот репозиторий — только каталог (`.claude-plugin/marketplace.json`): он перечисляет три плагина и указывает, из каких репозиториев их брать; при установке файлы скилла подтягиваются из его собственного репозитория, копий здесь нет. Каталог закрепляет проверенный коммит каждого скилла, а робот (`.github/workflows/sync.yml`) каждые шесть часов сверяет закрепления с репозиториями скиллов и **открывает PR с обновлением**, если исходный репозиторий ушёл вперёд. Изменение попадает в каталог только после проверки CI и ручного merge.
...
```bash
python -m scripts.validate_marketplace --check-docs
python -m unittest discover -s tests -p 'test_*.py' -v
python -m scripts.sync_plugins --check       # проверка актуальности, без записи
python -m scripts.sync_plugins --dry-run     # показать обновления, без записи
```

Команды `--check` и `--dry-run` требуют доступа к GitHub. Синхронизация не изменяет manifest, если хотя бы один удалённый репозиторий недоступен или вернул некорректный SHA. Для автоматического PR GitHub Actions должен иметь разрешение `Pull requests: Write` и настройку репозитория, допускающую создание pull request через `GITHUB_TOKEN`.
